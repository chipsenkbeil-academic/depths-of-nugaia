public var fractureToPoint = false;
public var totalMaxFractures = 3;
public var forcePerDivision = 20.0;
public var minBreakingForce = 0.0;
public var maxFracturesPerCall = 3;
public var randomOffset = 0.0;
public var minFractureSize : Vector3 = Vector3.zero;
public var grain : Vector3 = Vector3.one;
public var useCollisionDirection = 0.0;
public var fractureAtCenter = false;
public var smartJoints = false;
public var destroyAllAfterTime = 0.0;
public var destroySmallAfterTime = 0.0;
public var instantiateOnBreak : GameObject;
public var totalMassIfStatic = 1.0;
...
function OnCollisionEnter (collision : Collision) {
	//AudioSource.PlayClipAtPoint(clip, Vector3 (-62, -4, 97));
...
function Fracture (point : Vector3, force : Vector3, iterations : float) {
	...
	while (iterations > 0) {
		...
		sub = transform.worldToLocalMatrix.MultiplyVector(force.normalized) * useCollisionDirection * 
			Vector3.Dot(transform.worldToLocalMatrix.MultiplyVector(force.normalized), vec);
		...
		// IMPORTANT: Redo if we have a problem splitting; without this, we will get a lot of non-manifold meshes, 
		// convexhull errors and maybe even crash the game.
		if (broken == 0 || broken == gameObject.GetComponent(MeshFilter).mesh.vertexCount) {
			totalMaxFractures += 1;
			iterations += 1;
			Destroy(newObject);
			// This yield is here JUST so that when a large amount of objects are being broken, the screen doesn't freeze 
			// for a long time. It allows the screen to refresh before we're finnished, but if you don't, it might slow 
			// the script down trying to break a loop of bad planes.
			yield;
		}
		// If all's fine, apply the changes to each mesh
			...
			if (smartJoints) {
				...
				if (jointsb){
					// Basically, it goes through each joint and sees if the object A or B are closer to the 
					// connected body. Whichever is closer keeps the joint.
					for (i=0; i < jointsb.length; i++){
						if (jointsb[i].connectedBody != null && plane.GetSide(
						transform.worldToLocalMatrix.MultiplyPoint(jointsb[i].connectedBody.transform.position))) {
			...
			if (destroyAllAfterTime >= 0.1) {
				Destroy(newObject.GetComponent(MeshCollider),destroyAllAfterTime);
				Destroy(GetComponent(MeshCollider),destroyAllAfterTime);
